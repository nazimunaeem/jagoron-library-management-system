...
<title>দাতা সনদ — <?= htmlspecialchars($d['name']) ?></title>
...
.cert{border-color:#333!important;-webkit-print-color-adjust:exact;print-color-adjust:exact}}
</style>
</head><body>
<div class="no-print">
  <button onclick="window.print()" class="btn btn-primary">🖨️ সনদ প্রিন্ট করুন</button>
  <a href="donor_statement.php?id=<?= $id ?>" class="btn btn-outline">📄 বিবরণী (Statement)</a>
  ...
  <div class="cert-body">
    ...
    <div class="contrib-box">
      ...
      <div class="contrib-item">
        <strong>৳<?= number_format($d['money_total']) ?></strong>
        <span>নগদ অর্থ দান</span>
      </div>
      <?php endif; ?>
      <?php if ($d['book_total'] > 0): ?>
      <div class="contrib-item">
        <strong><?= $d['book_total'] ?>টি</strong>
        <span>বই দান</span>
      </div>
      <?php endif; ?>
      <?php if ($d['grand_total'] > 0 && $d['money_total'] > 0 && $d['book_total'] > 0): ?>
      <div class="contrib-item">
        <strong>৳<?= number_format($d['grand_total']) ?>+</strong>
